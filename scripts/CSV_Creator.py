...
url_test = f"{base_url}{dates[0]}{asset_id}"
response_test = session.get(url_test)
...
with ThreadPoolExecutor(max_workers=min(8, len(dates))) as executor:
    futures = [executor.submit(fetch_date_data, date) for date in dates]
    for future in futures:
        date, data = future.result()
        results_by_date[date] = data
...
with open(csv_file, "w") as file:
    ...
    for date in dates:
        ...
        key_find = [
            ...
            first_item.get("noncomm_positions_long_all"),
            first_item.get("noncomm_positions_short_all"),
            first_item.get("change_in_noncomm_long_all"),
            first_item.get("change_in_noncomm_short_all"),
        ]
